...
local function releaseMenuLockBySource(src)
    for shopName, holder in pairs(menuLocks) do
        if holder == src then
            menuLocks[shopName] = nil
        end
    end
end
...
local function GeneratePlate()
    while true do
        local plate = PlateUtils.GenerateFromConfig()
...
local function GetNearestShopByCoords(coords)
    ...
    for shopName, shopData in pairs(Config.Shops) do
        if shopData.Location then
            local dist = #(coords - vector3(shopData.Location.x, shopData.Location.y, shopData.Location.z))
...
-- Callbacks
QBCore.Functions.CreateCallback('qb-vehicleshop:server:acquireMenuLock', function(source, cb, shopName)
    if Config.SingleUserShopMenu == false then
        cb(true, nil)
        return
    end
    ...
    local holder = menuLocks[shopName]
    if not holder or holder == source or not QBCore.Functions.GetPlayer(holder) then
        menuLocks[shopName] = source
        cb(true, nil)
        return
    end

    local holderPlayer = QBCore.Functions.GetPlayer(holder)
    local holderName = nil
    if holderPlayer and holderPlayer.PlayerData and holderPlayer.PlayerData.charinfo then
        holderName = (holderPlayer.PlayerData.charinfo.firstname or '') .. ' ' .. (holderPlayer.PlayerData.charinfo.lastname or '')
        holderName = holderName:gsub('^%s+', ''):gsub('%s+$', '')
        if holderName == '' then holderName = nil end
    end
...
RegisterNetEvent('qb-vehicleshop:server:releaseMenuLock', function(shopName)
    if Config.SingleUserShopMenu == false then
        return
    end

    local src = source
    if shopName and menuLocks[shopName] == src then
        menuLocks[shopName] = nil
        return
    end
    releaseMenuLockBySource(src)
...
RegisterNetEvent('qb-vehicleshop:server:buyShowroomVehicle', function(vehicle)
    ...
    if not shopName then
        shopName = GetNearestShopByCoords(GetEntityCoords(GetPlayerPed(src)))
    end

    if type(vehicle) ~= 'string' or not QBCore.Shared.Vehicles[vehicle] then
        TriggerClientEvent('QBCore:Notify', src, Lang:t('error.vehnotfound'), 'error')
        return
    end
    ...
    if cash > tonumber(vehiclePrice) then
        ...
        TriggerClientEvent('qb-vehicleshop:client:buyShowroomVehicle', src, vehicle, plate, shopName)
...
RegisterNetEvent('qb-vehicleshop:server:financeVehicle', function(downPayment, paymentAmount, vehicle, shopName)
    local src = source
    if type(downPayment) == 'table' then
        local payload = downPayment
...
RegisterNetEvent('qb-vehicleshop:server:sellShowroomVehicle', function(data, playerid)
    ...
    if #(GetEntityCoords(GetPlayerPed(src)) - GetEntityCoords(GetPlayerPed(target.PlayerData.source))) < 3 then
        ...
        local shopName = GetNearestShopByCoords(GetEntityCoords(GetPlayerPed(src)))
        ...
        if cash >= tonumber(vehiclePrice) then
            ...
            TriggerClientEvent('qb-vehicleshop:client:buyShowroomVehicle', target.PlayerData.source, vehicle, plate, shopName)
